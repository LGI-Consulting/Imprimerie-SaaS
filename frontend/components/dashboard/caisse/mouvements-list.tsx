...
import { MouvementCaisse } from "@/lib/api/types"
...
export function MouvementsList({ caisseId }: MouvementsListProps) {
  const [mouvements, setMouvements] = useState<MouvementCaisse[]>([])
  const [loading, setLoading] = useState(true)
  const [dateDebut, setDateDebut] = useState<Date>()
  const [dateFin, setDateFin] = useState<Date>()

  const columns: ColumnDef<MouvementCaisse>[] = [
    {
    ...
    },
    {
      accessorKey: "montant",
      header: "Montant",
      cell: ({ row }) => caisse.formatAmount(row.getValue("montant"))
    },
    {
    ...
    },
    {
      ...
      header: "Solde après",
      cell: ({ row }) => caisse.formatAmount(row.getValue("solde_apres"))
    }
  ]
  ...
  const fetchMouvements = async () => {
    try {
      setLoading(true)
      const data = await caisse.getHistorique(
        caisseId,
